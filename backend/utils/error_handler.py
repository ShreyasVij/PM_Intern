...
def handle_api_error(error):
    ...
    app_logger.warning(f"API Error: {error.message} - Status: {error.status_code}")
    return response

def handle_generic_error(error):
    """Handle generic exceptions."""
    app_logger.error(f"Unhandled error: {str(error)}\n{traceback.format_exc()}")
    response = jsonify({
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                app_logger.error(f"Database error in {operation}: {str(e)}")
